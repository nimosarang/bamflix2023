...
function isValid(target, fieldName, focusTarget) {
  ...
  const elementType = (target.type === 'text' || target.type === 'password'
      || target.type === 'search' || target.type === 'textarea') ? '입력' : '선택';
 ...
 */
function callApi(uri, method, params, successCallback) {
  $.ajax({
    url: uri,
    type: method,
    contentType: 'application/json; charset=utf-8',
